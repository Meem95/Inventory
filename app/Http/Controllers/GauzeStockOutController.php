...
class GauzeStockOutController extends Controller
{
    public function index()
    {
        ...
        if($auth_user == "Admin"){
            $productPosSales = ProductSale::where('sale_type','Gauze Cloth')->latest()->get();
        }else{
            $productPosSales = ProductSale::where('sale_type','Gauze Cloth')->where('user_id',$auth_user_id)->latest()->get();
        }
        //dd($productPosSales);
        return view('backend.gauzeCloth.index',compact('productPosSales'));
    }
    ...
    public function create()
    {
        ...
        $products = DB::table('product_purchase_details')
            ...
            ->where('products.product_type','=','Gauze Cloth')
            ->groupBy('product_purchase_details.product_id')
            ->groupBy('product_purchase_details.barcode')
            ->latest('products.id')->get();
//dd($products);
        return view('backend.gauzeCloth.create',compact('parties','stores','products','productCategories','productSubCategories','productBrands'));
    }
...
//        dd(Cart::content());
        ...
        $productSale->sale_type = 'Gauze Cloth';
        $productSale->save();
