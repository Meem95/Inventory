...
class ProductPurchaseController extends Controller
{
    ...
    {
        //$productPurchases = ProductPurchase::where('purchase_product_type','Dress')->latest()->get();
    ...
    public function create()
    {
        ...
        //$products = Product::where('product_type','Dress')->get();
        $products = Product::latest()->get();
       // dd($products);
        return view('backend.productPurchase.create',compact('parties','stores','products','productCategories','productSubCategories','productBrands','productUnits'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $productPurchase ->purchase_product_type = 'Dress';
...
//                $barcode = Product::where('id',$product_id)->pluck('barcode')->first();
                ...
                $purchase_purchase_detail->rack_no = $request->rack_no[$i];
...
//                $purchase_purchase_detail->barcode = $barcode;
                ...
                //$stock->stock_product_type = 'Dress';
                $stock->stock_product_type = NULL;
            ...
            //$transaction->transaction_product_type = 'Dress';
            $transaction->transaction_product_type = NULL;
...
//            $purchase_purchase_detail->barcode = $barcode;
            ...
            $check_previous_stock = Stock::where('id','!=',$stock_id)->pluck('current_stock')->first();
            if(!empty($check_previous_stock)){
                $previous_stock = $check_previous_stock;
            }else{
                $previous_stock = 0;
...
//            'phone'=> 'required',
    ...
    public function partyBalance()
    {
        ...
        return view('backend.productPurchase.party_balance',compact('productPurchases'));
    }
    public function partyBalanceShow(Request $request)
    {
       //dd($request->all());
        $party_name = $request->party_id;
        $date_from = $request->date_from;
        $date_to = $request->date_to;

        if( (!empty($party_name)) && (!empty($date_from)) && (!empty($date_to)) )
        {
            $gl_pre_valance_data = DB::table('product_purchases')
                ->select('party_id', DB::raw('SUM(total_amount) as amount'))
                ->where('date', '<',$date_from)
                ->where('party_id',$party_name)
                ->groupBy('party_id')
                ->first();
        }else{
            $gl_pre_valance_data = DB::table('product_purchases')
        ...
        {
            //echo 'okk';exit;
                $total_amounts = DB::table('product_purchases')
                //->join('accounts', 'postings.id', '=', 'accounts.user_id')
                //->leftJoin('accounts', 'postings.account_no', '=', 'accounts.HeadCode')
                ->where('product_purchases.party_id',$party_name)
                ->whereBetween('product_purchases.date', [$date_from, $date_to])
        ...
        //dd($total_amounts);
        return view('backend.productPurchase.party_balance_show',compact('total_amounts','date_from','date_to','gl_pre_valance_data'));
    }
}
